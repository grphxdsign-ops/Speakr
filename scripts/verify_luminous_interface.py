...
QML_RUNTIME_WARNING_PATTERNS = (
    re.compile(
        r"(?im)^.*\.qml:\d+(?::\d+)?:\s*"
        r"(?:TypeError|ReferenceError|RangeError|Error):.*$"
    ),
    re.compile(r"(?im)^.*\bbinding loop detected\b.*$"),
    re.compile(
        r"(?im)^.*\.qml:\d+(?::\d+)?:.*\b"
        r"(?:deprecated|cannot read propert|cannot read properties|"
        r"unable to assign|is not a type)\b.*$"
    ),
)
...
def verification_commands(output: Path) -> list[tuple[str, Sequence[str], dict[str, str]]]:
    ...
    return [
        ...
        (
            "qmllint",
            (_qmllint_executable(), "-I", str(qml_dir), *qml_files),
            {},
        ),
...
def run_verification(output: Path) -> int:
    ...
    for index, (name, command, overrides) in enumerate(
        verification_commands(output), start=1
    ):
    ...
    _write_report(report_path, report)
    print(f"Verification passed. Evidence: {output}")
    return 0
